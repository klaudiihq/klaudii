 ...
 * @returns {{ mode, sessionNum, draft, beadId?, type? }}  sessionNum and draft are for the current mode
 */
function getWorkspace(workspace) {
  ...
  if (ws.beadId) result.beadId = ws.beadId;
  if (ws.type) result.type = ws.type;
  return result;
}

/**
 * Associate a workspace with a bead ID and optionally mark its type.
 * @param {string} workspace
 * @param {string} beadId
 * @param {string} [type] - "worker" or "user"
 */
function setBeadId(workspace, beadId, type) {
  const ws = ensureWorkspace(workspace);
  ws.beadId = beadId;
...
/**
 * Get the bead ID associated with a workspace (or null).
 */
function getBeadId(workspace) {
  const ws = state[workspace];
  return ws ? ws.beadId || null : null;
}

/**
 * Find all workspaces associated with a given bead ID.
 * @returns {string[]} workspace names
 */
function getWorkspacesForBead(beadId) {
  return Object.keys(state).filter(ws => state[ws].beadId === beadId);
}
...
const cumulativeStats = new Map(); // workspace → { cost, input_tokens, output_tokens, total_tokens }
...
function addTurnStats(workspace, stats) {
  if (!stats) return;
  const cur = cumulativeStats.get(workspace) || { cost: 0, input_tokens: 0, output_tokens: 0, total_tokens: 0 };
  if (stats.cost != null) cur.cost += stats.cost;
  if (stats.input_tokens) cur.input_tokens += stats.input_tokens;
...
function getCumulativeStats(workspace) {
  return cumulativeStats.get(workspace) || { cost: 0, input_tokens: 0, output_tokens: 0, total_tokens: 0 };
}
...
module.exports = { getWorkspace, setState, setBeadId, getBeadId, getWorkspacesForBead, touchChatActivity, getLastChatActivity, setStreaming, isStreaming, setPendingPermission, getPendingPermission, validModes, setWorkspaceType, getWorkspaceType, addTurnStats, getCumulativeStats, resetCumulativeStats };
