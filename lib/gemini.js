 ...
 * Message sending is handled by gemini-core.js (in-process gemini-cli-core).
 */
...
const core = require("./gemini-core");
...
function startChat(workspace, workspacePath, userMessage, config, opts = {}) {
  ...
  return core.startChat(workspace, sessionNum, workspacePath, userMessage, config, opts);
}

/** Kill active Gemini session for a workspace (or specific session). */
function stopProcess(workspace, sessionNum) {
  core.stopProcess(workspace, sessionNum);
}
...
/** Check if a Gemini session is currently active for a workspace (or specific session). */
function isActive(workspace, sessionNum) {
  return core.isActive(workspace, sessionNum);
}
...
function getSessions(workspace) {
  ...
  const details = nums.map(num => {
    ...
    return {
      num,
      lastActivity,
      active: core.isActive(workspace, num),
    };
  });
...
/** Kill all active Gemini sessions (for graceful shutdown). */
function stopAllProcesses() {
  core.stopAllProcesses();
}
...
module.exports = {
  ...
  confirmToolCall(workspace, sessionNum, callId, outcome, answer) {
    return core.confirmToolCall(workspace, sessionNum, callId, outcome, answer);
  },
  executeCommand(workspace, sessionNum, command, args) {
    return core.executeCommand(workspace, sessionNum, command, args);
  },
};
