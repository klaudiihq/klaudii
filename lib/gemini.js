...
// --- A2A backend (unconditional) ---
const a2a = require("./gemini-a2a");
...
function startChat(workspace, workspacePath, userMessage, config, opts = {}) {
  return a2a.startChat(workspace, workspacePath, userMessage, config, opts);
}

/** Kill active Gemini A2A server for a workspace. */
function stopProcess(workspace) {
  a2a.stopProcess(workspace);
}
...
/** Check if a Gemini A2A server is currently running for a workspace. */
function isActive(workspace) {
  return a2a.isActive(workspace);
}
...
module.exports = {
  ...
  getQuota,
  startQuotaRefresh,
  confirmToolCall(workspace, callId, outcome, answer) {
    return a2a.confirmToolCall(workspace, callId, outcome, answer);
  },
};
