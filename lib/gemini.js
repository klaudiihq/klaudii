...
// --- A2A backend (optional) ---
// If @google/gemini-cli-a2a-server is installed, use it for all message sending.
// Falls back to print-mode subprocess if not available.
let a2a = null;
try {
  require.resolve("@google/gemini-cli-a2a-server/package.json");
  a2a = require("./gemini-a2a");
  log("A2A server package found — using A2A backend for messaging");
} catch {
  log("A2A server package not found — using print-mode subprocess backend");
}
...
function streamLogPath(workspace) {
  return path.join(CONVERSATIONS_DIR, `stream-gemini-${workspace}.jsonl`);
}
...
function startChat(workspace, workspacePath, userMessage, config, opts = {}) {
  if (a2a) return a2a.startChat(workspace, workspacePath, userMessage, config, opts);
  ...
  rl.on("line", (line) => {
    ...
    try {
      ...
      // Write-through: durable log for crash recovery
      try { fs.appendFileSync(logPath, trimmed + '\n'); } catch {}
  ...
  proc.on("close", (code) => {
    ...
    // Only delete stream log when doneCallback has persisted history (normal exit).
    // When killed, leave it so recoverStreams() can persist on next startup.
    if (!entry.killed) {
      try { fs.unlinkSync(logPath); } catch {}
    }
  });
...
function stopProcess(workspace) {
  if (a2a) { a2a.stopProcess(workspace); return; }
...
function isActive(workspace) {
  if (a2a) return a2a.isActive(workspace);
...
function getSessions(workspace) {
  ...
  const details = nums.map(num => {
    ...
    return {
      num,
      lastActivity,
      active: active && num === entry.current,
    };
  });
...
/**
 * Return the accumulated assistant text from an active stream log (non-destructive).
 * Used by the /api/gemini/stream-partial endpoint so clients can show partial
 * content when switching back to a workspace mid-stream.
 * Returns null when no stream log exists.
 */
...
function stopAllProcesses() {
  if (a2a) { a2a.stopAllProcesses(); return; }
...
module.exports = {
  ...
  getStreamPartial,
  recoverStreams,
  confirmToolCall(workspace, callId, outcome) {
    if (a2a) return a2a.confirmToolCall(workspace, callId, outcome);
    throw new Error("confirmToolCall requires A2A backend");
  },
};
