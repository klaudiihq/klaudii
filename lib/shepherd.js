...
function buildSessionList(ctx) {
  ...
  return allProjects.map((project) => {
    ...
    const beadId = ctx.workspaceState ? ctx.workspaceState.getBeadId(project.name) : null;
...
function run(ctx) {
  ...
  for (const ws of running) {
    ...
    if (ws.lastActivity && idleMs > STUCK_THRESHOLD_MS) {
      ...
      // Prefer explicit beadId from workspace state, fall back to branch name regex
      const beadId = (ws.beadId) ||
        (ws.git && ws.git.branch ? (ws.git.branch.match(/^bead-(.+)$/) || [])[1] : null) ||
        null;
  ...
  for (const bead of readyBeads) {
    ...
    // Tag workspace with bead ID for tracking
    if (inProcess && ctx.workspaceState) {
      ctx.workspaceState.setBeadId(result.project, bead.id, "worker");
    }
    // Tag the chat session with bead ID
    if (inProcess && ctx.claudeChat) {
      ctx.claudeChat.tagSessionWithBead(result.project, 1, bead.id);
    }
  ...
  const settingsPath = path.join(require("os").homedir(), "Library", "Application Support", "com.klaudii.server", "settings.json");
