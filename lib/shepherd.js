...
function run(ctx) {
  ...
  // Helper: create new workspace
  function createWorkspace(repo, branch) {
    if (inProcess && ctx.config && ctx.config.reposDir) {
      try {
        ...
        const claudeArgs = "remote-control --permission-mode bypassPermissions";
        ctx.tmux.createSession(tmuxName, worktreeDir, claudeArgs);
        ...
        if (ctx.sessionTracker) {
          ...
          ctx.sessionTracker.captureClaudeUrl(projectName, tmuxName).catch(() => {});
        }

        return { ok: true, project: projectName, worktree: worktreeDir, branch: branchName };
      } catch (err) {
        console.error(`[shepherd] createWorkspace failed:`, err.message);
        return null;
      }
    }
    return httpApi("POST", "/api/sessions/new", { repo, branch });
  }

  // Helper: send message to workspace
  function sendMessage(workspace, message, sender) {
    if (inProcess && ctx.claudeChat) {
      try {
        ctx.claudeChat.pushHistory(workspace, "user", message, { sender });
        return { ok: true };
      } catch (err) {
        console.error(`[shepherd] sendMessage failed:`, err.message);
        return null;
      }
    }
    return httpApi("POST", `/api/chat/${encodeURIComponent(workspace)}/send`, { message, sender });
  }
  ...
  for (const bead of readyBeads) {
    ...
    const result = createWorkspace(REPO_NAME, branchName);
    if (!result || !result.ok) {
    ...
    sendMessage(workspace, workerPrompt, "shepherd");
