...
function run(ctx) {
  ...
  for (const bead of readyBeads) {
    ...
    const workerPrompt = [
      ...
      `Run: bd show ${bead.id} --allow-stale to get the full spec.`,
      `Claim it: bd update ${bead.id} --claim --allow-stale.`,
      `Do the work per the spec. Verify per the spec.`,
      `Close it: bd close ${bead.id} --reason Done --allow-stale.`,
      `When done, commit and push to origin main. Use bd export -o .beads/issues.jsonl --allow-stale before committing.`,
    ].join("\n");
