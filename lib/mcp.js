...
function createMcpServer(deps) {
  ...
  server.tool(
    ...
    async ({ repo, branch, task_id }) => {
      ...
      const { projectName, workspacePath: worktreeDir } = wsProvider
        ...
        : { projectName: `${repo}--${branchName}`, workspacePath: require("path").join(config.reposDir, `${repo}--${branchName}`) };
      ...
      if (task_id) {
        try {
          ...
          execSync(`bd update ${task_id} --status in_progress --json`, { encoding: "utf-8", cwd });
          execSync(`bd comment ${task_id} ${JSON.stringify(`Assigned to workspace ${projectName}`)}`, { encoding: "utf-8", cwd });
        } catch {}
      }
  ...
  server.tool(
    ...
    async ({ status, priority }) => {
      try {
        ...
        if (status) tasks = tasks.filter((b) => b.status === status);
        if (priority !== undefined) tasks = tasks.filter((b) => b.priority === priority);
  ...
  server.tool(
    ...
    async ({ id, status, comment, assignee }) => {
      if (!/^[a-zA-Z0-9-]+$/.test(id)) {
        return { content: [{ type: "text", text: JSON.stringify({ error: "invalid task ID" }) }], isError: true };
      }
  ...
  // --- klaudii_complete_task ---

  server.tool(
    "klaudii_complete_task",
    "Run the completion pipeline for a task: tests → verification → code review → close. Call this when work on a task is done.",
    {
      task_id: z.string().describe("Task ID (e.g. 'klaudii-abc')"),
      workspace: z.string().describe("Workspace name where the work was done"),
    },
    async ({ task_id, workspace }) => {
      if (!/^[a-zA-Z0-9-]+$/.test(task_id)) {
        return { content: [{ type: "text", text: JSON.stringify({ error: "invalid task ID" }) }], isError: true };
      }
      ...
      try {
        const result = await completion.runPipeline(task_id, workspace, ctx);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      } catch (err) {
