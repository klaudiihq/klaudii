...
/**
 * Discover all running Claude processes on this machine.
 * managedPids = PIDs from tmux panes (direct children of tmux).
 * A claude process is "managed" if its PID or any ancestor PID is in managedPids.
 */
function findClaudeProcesses(managedPids) {
  ...
  // Find all top-level claude processes (not child sdk processes)
  const claudeProcs = [];
  for (const line of lines) {
    const match = line.trim().match(/^(\d+)\s+(\d+)\s+([\d.]+)\s+(\d+)\s+([\d:.-]+)\s+(.*)$/);
    if (!match) continue;
    ...
    // Match "claude" commands but not node/tmux/ttyd/KlaudiiMenu/grep/shell wrappers
    if (!command.includes("claude")) continue;
    if (/node|tmux|ttyd|KlaudiiMenu|grep|pgrep|shell-snapshots/.test(command)) continue;

    // Skip child sdk processes (they have --sdk-url), we only want the parent claude process
    if (command.includes("--sdk-url")) continue;

    claudeProcs.push({ pid, ppid, etime: etime.trim(), command: command.trim() });
  }
