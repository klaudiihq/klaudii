...
function findClaudeProcesses(managedPids) {
  ...
  // Build commandMap from initial ps lines for findAncestorApp (avoids per-process ps call)
  const commandMap = {};
  for (const line of lines) {
    const m = line.trim().match(/^(\d+)\s+\d+\s+[\d.]+\s+\d+\s+[\d:.-]+\s+(.*)$/);
    if (m) commandMap[m[1]] = m[2];
  }

  // Single batched lsof for all process CWDs (replaces N per-process execSync calls)
  const cwdMap = {};
  if (allProcs.length > 0) {
    try {
      const pidList = allProcs.map(p => p.pid).join(",");
      const lsofOut = execSync(`lsof -a -d cwd -p ${pidList} 2>/dev/null`, { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] });
      for (const line of lsofOut.trim().split("\n").slice(1)) { // skip header row
        const parts = line.trim().split(/\s+/);
        // lsof output: COMMAND PID USER FD TYPE DEVICE SIZE/OFF NODE NAME
        if (parts.length >= 9) cwdMap[parts[1]] = parts[parts.length - 1];
      }
    } catch {}
  }
  ...
  for (const proc of allProcs) {
    ...
    // Walk ancestors to find the launching app (pass pre-built commandMap)
    const launchedBy = findAncestorApp(proc.pid, ppidMap, commandMap);
 ...
 * commandMap is pid → full command string (from the initial ps call).
 */
function findAncestorApp(pid, ppidMap, commandMap) {
  ...
  for (let i = 0; i < 15; i++) {
    const parent = ppidMap[current];
    if (!parent || parent === "0" || parent === "1" || parent === current) break;
    const cmd = commandMap[parent] || "";

    // Match .app bundle path (full command line contains the path)
