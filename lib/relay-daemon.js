...
function handleLine(line) {
  ...
  try { JSON.parse(t); } catch { rlog(`claude non-json: ${t.slice(0, 300)}`); return; }
  eventCount++;
  ...
  if (!CLOSE_STDIN) {
    ...
    rl2.on("line", (line) => {
      ...
      if (t) {
        ...
        try { proc.stdin.write(t + "\n"); } catch (e) { rlog(`stdin write error: ${e.message}`); }
      }
    });
  }
...
process.on("uncaughtException", (err) => {
  rlog(`uncaught exception: ${err.message}\n${err.stack}`);
