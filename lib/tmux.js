...
//
// Fallback: ~/.klaudii/tmux.sock if config doesn't specify one.
// setup.sh always writes tmuxSocket into config.json, so this only
// fires in dev environments without a config file.
// os.homedir() reads from passwd (not HOME env var), so it's safe under launchd.
let _configSocket, _tmuxBin;
try {
  const _cfg = require("./projects").loadConfig();
  _configSocket = _cfg.tmuxSocket;
  _tmuxBin = _cfg.tmuxPath || null;
} catch {}
const TMUX_SOCKET = _configSocket || path.join(os.homedir(), ".klaudii", "tmux.sock");
const TMUX_BIN = _tmuxBin || "tmux";
const TMUX = `${TMUX_BIN} -S '${TMUX_SOCKET}'`;

function isTmuxInstalled() {
  try {
    execSync(`which ${TMUX_BIN}`, { stdio: "pipe" });
    return true;
  } catch {
    return false;
  }
