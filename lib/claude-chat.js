...
function connectRelay(workspace, { replayCallback, replayDoneCallback } = {}) {
  ...
  activeRelays.set(workspace, { handle, killed: false });
    ...
    case "assistant": {
      ...
      for (const block of msg.content) {
        if (block.type === "text" && block.text) {
          ...
          if (/ask.*question|askfollowup|ask_followup/i.test(block.name)) {
            console.log(`[claude-chat] ask-tool raw input: name=${block.name} input=${JSON.stringify(block.input)}`);
          }
          events.push({
            type: "tool_use",
  ...
  const msg = JSON.stringify({
    ...
    response: {
      ...
      response: behavior === "allow"
        ? { behavior: "allow", updatedInput: updatedInput || undefined }
