...
const SESSIONS_FILE_OLD = path.join(require("os").homedir(), "Library", "Application Support", "com.klaudii.server", "claude-chat-sessions.json");
...
async function startChat(workspace, workspacePath, userMessage, config, opts = {}) {
  ...
  if (sessionId) {
    ...
    // No session to resume — inject conversation history as system prompt context
    // so the new Claude instance has continuity from previous sessions.
    // Using --append-system-prompt instead of prepending to the user message
    // avoids Claude treating the briefing as "something to respond to" (which
    // triggers re-reading files, re-orienting, etc). System prompt = "you already know this."
...
function newSession(workspace) {
  ...
  // Update workspace-state to point to the new session
  if (workspaceStateRef) {
