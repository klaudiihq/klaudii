...
function sendMessage(workspace, workspacePath, userMessage, config, opts = {}) {
  ...
  // bypassPermissions fast path: -p + plain text stdin + EOF (unchanged behavior)
  // plan/allowEdits: bidirectional JSONL protocol — no -p, stdin stays open for control responses
  ...
  if (interactive) {
    // Bidirectional JSONL: send user message as a JSON object, keep stdin open for control responses
    ...
    // stdin stays open — sendControlResponse() will write to it
  } else {
    // Fast path: plain text prompt followed by EOF
  ...
  rl.on("line", (line) => {
    ...
    try {
      ...
      log(`raw-event workspace=${workspace} #${eventCount} type=${raw.type}${raw.subtype ? " subtype=" + raw.subtype : ""}${raw.request?.subtype ? " req=" + raw.request.subtype : ""}`);

      // Handle bidirectional control requests (plan/allowEdits modes)
      if (raw.type === "control_request") {
        ...
        if (req.subtype === "can_use_tool") {
          ...
          log(`control_request subtype=${req.subtype} request_id=${raw.request_id} — ignoring`);
        }
        return; // control_request events are not normalized or logged to history
      }
...
function sendControlResponse(workspace, requestId, behavior, updatedInput) {
  ...
  if (!entry || !entry.proc || entry.killed) {
    log(`sendControlResponse workspace=${workspace} no active process`);
    return;
  }
...
function sendToolResult(workspace, toolId, content) {
  const entry = activeProcesses.get(workspace);
  if (!entry || !entry.proc || entry.killed) {
    log(`sendToolResult workspace=${workspace} no active process`);
    return;
  }
  const msg = JSON.stringify({
    type: "tool_result",
