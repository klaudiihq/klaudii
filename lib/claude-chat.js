...
const { DATA_DIR, RELAY_DIR: PATHS_RELAY_DIR, CHATS_DIR } = require("./paths");
...
function connectRelay(workspace, { sessionNum, replayCallback, replayDoneCallback } = {}) {
  ...
  let currentSocket = null;         // always points to the live socket
  let heartbeatTimer = null;
  let destroyed = false;            // set by handle.kill() — prevents auto-reconnect
  ...
  // heartbeats every 15s), the relay is hung — reconnect if daemon is alive.
  function resetHeartbeat() {
    if (heartbeatTimer) clearTimeout(heartbeatTimer);
    heartbeatTimer = setTimeout(() => {
      if (destroyed) return;
      logErr(`relay heartbeat timeout workspace=${workspace} — attempting reconnect`);
      if (currentSocket) currentSocket.destroy();
      if (isRelayAlive(workspace, sNum)) {
        log(`relay daemon alive, reconnecting workspace=${workspace}`);
        attachSocket(true);
      } else {
        logErr(`relay daemon dead workspace=${workspace}`);
        deleteRelay(workspace, sNum);
        partialStreams.delete(pKey);
        if (errorCallback) errorCallback(new Error("relay heartbeat timeout: daemon died"));
      }
    }, 45000);
    heartbeatTimer.unref();
  }

  // Create a new socket and wire up all handlers.
  // isReconnect=true: silently consume the relay's replay phase (already processed),
  // then resume handling live events as if nothing happened.
  function attachSocket(isReconnect) {
    const socket = new net.Socket();
    currentSocket = socket;
    let socketErrorFired = false;

    // On reconnect the relay always sends a full replay before live events.
    // Suppress it — we already have the state from the original connection.
    let suppressingReplay = isReconnect;
    if (isReconnect) replayDone = false;

    setRelay(workspace, sNum, { handle, killed: false, startedAt: Date.now() });

    socket.connect(rp.socket, () => {
      log(`relay ${isReconnect ? "re" : ""}connected workspace=${workspace}`);
      resetHeartbeat();
    });
    ...
    rl.on("line", (line) => {
      ...
      if (!replayDone) {
        if (raw.type === "relay_replay_end") {
          ...
          log(`relay replay done workspace=${workspace}${isReconnect ? " (reconnect, suppressed)" : ""}`);
          if (!isReconnect) {
            // First connection: normal replay done handling
        ...
        // During reconnect replay: silently discard everything
      ...
      if (raw.type === "relay_exit") {
        ...
        deleteRelay(workspace, sNum);
        partialStreams.delete(pKey);
        if (doneCallback) doneCallback({ code: raw.code, stderr: raw.stderr || "" });
    ...
    socket.on("error", (err) => {
      ...
      if (destroyed) { deleteRelay(workspace, sNum); return; }
      // Try to reconnect if the relay daemon is still running
      if (isRelayAlive(workspace, sNum)) {
        log(`relay socket error but daemon alive, reconnecting workspace=${workspace}`);
        setTimeout(() => { if (!destroyed) attachSocket(true); }, 500);
      } else {
        deleteRelay(workspace, sNum);
        partialStreams.delete(pKey);
    ...
    socket.on("close", () => {
      ...
      if (destroyed) { deleteRelay(workspace, sNum); return; }
      if (!socketErrorFired) {
        // Unexpected clean close (relay crashed without relay_exit) — reconnect if alive
        if (isRelayAlive(workspace, sNum)) {
          log(`relay socket closed unexpectedly, reconnecting workspace=${workspace}`);
          setTimeout(() => { if (!destroyed) attachSocket(true); }, 500);
          return;
        }
        deleteRelay(workspace, sNum);
...
function findClaudeBin(config) {
  ...
  if (config && config.claudePath && fs.existsSync(config.claudePath)) {
    cachedBinPath = config.claudePath;
