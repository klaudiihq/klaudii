...
function normalizeEvent(raw) {
  ...
  switch (raw.type) {
    ...
    case "user": {
      ...
      // Marked _flush so the server persists history but does NOT broadcast "done"
      // or clear streaming — the model is still working.
      events.push({ type: "result", stats: {}, _flush: true });
      break;
    }
    ...
    case "result": {
      ...
      if (raw.usage) {
        const inputFull = (raw.usage.input_tokens || 0) +
          (raw.usage.cache_read_input_tokens || 0) + (raw.usage.cache_creation_input_tokens || 0);
        stats.total_tokens = inputFull + (raw.usage.output_tokens || 0);
        stats.input_tokens = inputFull;
...
async function startChat(workspace, workspacePath, userMessage, config, opts = {}) {
  ...
  if (sessionId) {
    ...
    // No session to resume — inject conversation history as context briefing
    // so the new Claude instance has continuity from previous sessions.
    const briefing = generateBriefing(workspace);
    if (briefing) {
      log(`startChat: injecting briefing for workspace=${workspace} briefingLen=${briefing.length}`);
      fullMessage = briefing + "\n\n---\n\nThe user's new message:\n\n" + fullMessage;
    }
  }
  ...
  log(`spawn workspace=${workspace} bin=${claudeBin} model=${opts.model || "auto"} permMode=${permMode} thinking=${!!opts.thinking} hasSession=${!!sessionId} images=${imagePaths.length}`);
...
function sendMessage(workspace, message) {
  const sNum = currentSessionNum(workspace);
  const entry = getRelay(workspace, sNum);
...
function tagSessionWithTask(workspace, sessionNum, taskId) {
  if (!sessions[workspace]) sessions[workspace] = { current: 1, sessions: {} };
  const entry = sessions[workspace];
  if (!entry.taskIds) entry.taskIds = {};
  const num = String(sessionNum || entry.current);
  if (!entry.taskIds[num]) entry.taskIds[num] = [];
  if (!entry.taskIds[num].includes(taskId)) {
    entry.taskIds[num].push(taskId);
    saveSessions();
  }
}
...
function getSessionsForTask(taskId) {
  ...
  for (const [workspace, entry] of Object.entries(sessions)) {
    if (!entry.taskIds) continue;
    for (const [num, ids] of Object.entries(entry.taskIds)) {
...
const HANDOFF_TAIL_TURNS = 20; // max user/assistant pairs to include in briefing

/**
 * Generate a briefing message from the current session's conversation history.
 * Used to prime a new Claude instance so it can seamlessly continue.
 */
function generateBriefing(workspace) {
  const history = getHistory(workspace);
  if (!history || history.length === 0) return null;

  // Extract user/assistant messages (skip tool_use/tool_result noise)
  const conversational = history.filter(m => m.role === "user" || m.role === "assistant");

  // Take the tail — recent context matters most
  const tail = conversational.slice(-HANDOFF_TAIL_TURNS * 2);

  // Format as a readable conversation transcript
  const transcript = tail.map(m => {
    const role = m.role === "user" ? "User" : "Assistant";
    const text = (m.content || "").slice(0, 2000); // truncate very long messages
    return `${role}: ${text}`;
  }).join("\n\n");
...
async function performHandoff(workspace, workspacePath, config, opts = {}) {
  ...
  const oldSession = currentSessionNum(workspace);
  stopSessionRelay(workspace, oldSession);

  // Create a new session (increments session number, no --resume)
  const newSNum = newSession(workspace);
  log(`handoff: new session#${newSNum} for workspace=${workspace}`);
  ...
  const rp = await startRelay(workspace, newSNum, workspacePath, claudeBin, args, env, initMsg, false);
  log(`handoff: relay ready workspace=${workspace} session#${newSNum}`);

  const handle = connectRelay(workspace, { sessionNum: newSNum });
  return { handle, sessionNum: newSNum };
}
