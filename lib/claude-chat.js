...
async function sendMessage(workspace, workspacePath, userMessage, config, opts = {}) {
  ...
  const interactive = permMode !== "bypassPermissions";

  const args = interactive
    ? ["--output-format", "stream-json", "--input-format", "stream-json", "--verbose",
       "--permission-prompt-tool", "stdio", "--permission-mode", permMode]
    : ["-p", "--output-format", "stream-json", "--verbose", "--permission-mode", permMode];
  ...
  // Build the initial stdin payload
  let initMsg;
  if (interactive) {
  ...
  log(`spawn workspace=${workspace} bin=${claudeBin} model=${opts.model || "auto"} permMode=${permMode} interactive=${interactive} hasSession=${!!sessionId} images=${imagePaths.length}`);

  // Start relay daemon — Claude runs inside it, detached from us
  const rp = await startRelay(workspace, workspacePath, claudeBin, args, env, initMsg, !interactive);
