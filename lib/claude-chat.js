...
function relayPaths(workspace) {
  const dir = path.join(RELAY_DIR, workspace);
  return {
    dir,
...
function isRelayAlive(workspace) {
  const { pid } = relayPaths(workspace);
  try {
 ...
 */
async function startRelay(workspace, workspacePath, bin, args, env, initMsg, closeStin) {
  const rp = relayPaths(workspace);
 ...
 */
function connectRelay(workspace, { replayCallback, replayDoneCallback } = {}) {
  const rp = relayPaths(workspace);
  ...
  const handle = {
    ...
    flushTurn() {
      ...
      partialStreams.delete(workspace);
      try { fs.unlinkSync(streamLogPath(workspace)); } catch {}
    },
  };
  ...
  activeRelays.set(workspace, { handle, killed: false, startedAt: Date.now() });
  ...
  rl.on("line", (line) => {
    ...
    if (!replayDone) {
      if (raw.type === "relay_replay_end") {
        ...
        const seedText = partialStreams.get(workspace);
      ...
      for (const evt of replayNorm) {
        if (evt.type === "message" && (evt.role === "assistant" || !evt.role)) {
          partialStreams.set(workspace, (partialStreams.get(workspace) || "") + (evt.content || ""));
    ...
    if (raw.type === "relay_exit") {
      log(`relay exit workspace=${workspace} code=${raw.code}`);
      activeRelays.delete(workspace);
      partialStreams.delete(workspace);
...
const partialStreams = new Map(); // workspace → string

function getStreamPartial(workspace) {
  return partialStreams.get(workspace) || null;
}
...
async function sendMessage(workspace, workspacePath, userMessage, config, opts = {}) {
  ...
  const rp = await startRelay(workspace, workspacePath, claudeBin, args, env, initMsg, false);

  log(`relay ready workspace=${workspace} socket=${rp.socket}`);
  ...
  const handle = connectRelay(workspace);
...
function sendControlRequest(workspace, subtype, payload = {}) {
  ...
  if (!entry) {
    log(`sendControlRequest workspace=${workspace} subtype=${subtype} no active relay`);
    return null;
  }
  ...
  log(`sendControlRequest workspace=${workspace} subtype=${subtype} requestId=${requestId}`);
...
function stopProcess(workspace) {
  ...
  const entry = activeRelays.get(workspace);
  if (entry) {
    log(`stop relay workspace=${workspace}`);
    entry.handle.kill();
  }
}
...
/** Return relay details for a workspace (pid, startedAt, sessionNum), or null. */
function getRelayInfo(workspace) {
  const entry = activeRelays.get(workspace);
  if (!entry) return null;
  const rp = relayPaths(workspace);
  let pid = null;
  try {
    pid = parseInt(fs.readFileSync(rp.pid, "utf-8").trim(), 10);
  } catch {}
  return {
    pid,
    startedAt: entry.startedAt || null,
    sessionNum: currentSessionNum(workspace),
  };
}
...
function appendMessage(workspace, message) {
  const entry = activeRelays.get(workspace);
  if (!entry) { logErr(`appendMessage: no active relay for workspace=${workspace}`); return false; }
...
function recoverStreams() {
  ...
  for (const file of files) {
    ...
    if (isRelayAlive(workspace)) {
      log(`recover: skipping ${workspace} — relay still alive`);
      continue;
    }
...
function reconnectActiveRelays(config, onWorkspace) {
  ...
  for (const ws of dirs) {
    if (!isRelayAlive(ws)) continue;
    log(`reconnecting to relay workspace=${ws}`);
    try {
      ...
      const handle = connectRelay(ws, {
        replayDoneCallback() {
          ...
          if (replayAssistantText) {
            const num = String(currentSessionNum(ws));
            ...
            if (lastMsg && lastMsg.role === "user") {
              ...
              log(`replay-flushed text-only turn for workspace=${ws} (${replayAssistantText.length} chars)`);
            }
          }
          ...
          try { fs.unlinkSync(streamLogPath(ws)); } catch {}
        },
        replayCallback(raw) {
          ...
          if (raw.type === "system" && raw.subtype === "init" && raw.session_id) {
            ...
            entry.sessions[String(entry.current)] = raw.session_id;
            saveSessions();
          }
          ...
          for (const evt of normalized) {
            if (evt.type === "message" && (evt.role === "assistant" || !evt.role)) {
              ...
              if (lastMsg && lastMsg.role === "user") {
                ...
                if (batch.length > 0) {
                  pushHistoryBatch(ws, batch);
                  log(`replay-recovered ${batch.length} event(s) for workspace=${ws}`);
                }
              }
              replayAssistantText = "";
      ...
      logErr(`reconnect failed workspace=${ws}: ${e.message}`);
    }
  }
}
...
function stopAllProcesses() {
  for (const workspace of [...activeRelays.keys()]) {
    stopProcess(workspace);
  }
}
