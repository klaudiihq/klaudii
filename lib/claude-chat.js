...
function sendMessage(workspace, workspacePath, userMessage, config, opts = {}) {
  ...
  log(`spawn workspace=${workspace} bin=${claudeBin} model=${opts.model || "auto"} hasSession=${!!sessionId} images=${imagePaths.length}`);
  ...
  // Write the message to stdin and close it — Claude reads the full prompt then responds.
