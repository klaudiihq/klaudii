...
function generateBriefing(workspace) {
  ...
  const tail = conversational.slice(-HANDOFF_CONVERSATION_PAIRS * 2);
  ...
  sections.push("Please continue the conversation from where we left off without asking the user any further questions. Continue with the last task that you were asked to work on.");
...
async function performHandoff(workspace, workspacePath, config, opts = {}) {
  ...
  // Don't switch workspace-state — the UI will switch when it processes the
  // context_reload broadcast. Otherwise we hijack the user's current view.
  const newSNum = newSession(workspace, { switchTo: false });
