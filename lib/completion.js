// Completion pipeline — 4-stage quality gate for task completion.
//
// Called when a worker announces it's done. Runs:
//   1. Tests — npm test (or task spec verification commands)
//   2. Verification agent — ephemeral Claude reviews work vs task spec
//   3. Code review agent — ephemeral Claude reviews the diff
//   4. Close task — only if all gates pass
//
...
async function runPipeline(taskId, workspace, ctx) {
  const { claudeChat, projects, config } = ctx;

  log(`starting pipeline task=${taskId} workspace=${workspace}`);

  // Get task spec
  const task = bd(`show ${taskId}`);
  if (!task) {
    return { ok: false, stage: "init", error: `Task ${taskId} not found` };
  }
  ...
  // ===== Stage 1: Run Tests =====
  log(`stage 1: tests task=${taskId}`);
  const testResult = runTests(workspacePath);

  if (!testResult.pass) {
    log(`stage 1 FAILED task=${taskId}`);
  ...
  log(`stage 1 PASSED task=${taskId} (${testResult.skipped ? "skipped" : "passed"})`);

  // ===== Stage 2: Verification Agent =====
  log(`stage 2: verification task=${taskId}`);
  const diff = getDiff(workspacePath);
  const truncDiff = diff.length > 50000 ? diff.slice(0, 50000) + "\n...(truncated)" : diff;

  const verifyPrompt = [
    "You are a verification agent. Review whether the code changes fulfill the task specification.",
    "",
    "TASK SPECIFICATION:",
  ...
  if (!verifyResult.ok) {
    logErr(`stage 2 ERROR task=${taskId}: ${verifyResult.error}`);
    bdRaw(`comment ${taskId} "Completion pipeline: verification agent failed to start — ${verifyResult.error}. Escalating to Shepherd."`);
  ...
  if (verifyParsed.verdict === "fail") {
    log(`stage 2 FAILED task=${taskId}`);
  ...
  if (!reviewResult.ok) {
    logErr(`stage 3 ERROR task=${taskId}: ${reviewResult.error}`);
    bdRaw(`comment ${taskId} "Completion pipeline: code review agent failed — ${reviewResult.error}. Escalating to Shepherd."`);
  ...
  if (egregiousIssues.length > 0) {
    log(`stage 3 FAILED task=${taskId} (${egregiousIssues.length} egregious issues)`);
  ...
  log(`stage 3 PASSED task=${taskId}`);

  // ===== Stage 4: Close Task =====
  log(`stage 4: closing task=${taskId}`);
  bdRaw(`close ${taskId} --reason "Completed — all quality gates passed"`);

  const summary = [
    `Task ${taskId} completed. All quality gates passed.`,
  ...
  log(`pipeline complete task=${taskId}`);
