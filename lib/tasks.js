// SQLite-backed task management — replaces beads/Dolt/bd CLI.
//
...
function autoImport() {
  ...
  const candidates = [
    "/tmp/beads-merged.jsonl",
    path.resolve(__dirname, "..", ".beads", "issues.jsonl"),
  ];

  for (const f of candidates) {
    if (fs.existsSync(f)) {
      const imported = importFromJSONL(f);
      if (imported > 0) {
        console.log(`[tasks] auto-imported ${imported} tasks from ${f}`);
        return imported;
      }
    }
  }
