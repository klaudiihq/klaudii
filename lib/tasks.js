...
//
// DB location: ~/.klaudii/data/klaudii.db
...
function getDb() {
  ...
  try {
    ...
    console.warn("[tasks] better-sqlite3 not installed — task features disabled");
    return null;
  }
...
function create({ title, description, priority, difficulty, type, project }) {
  const db = getDb();
  if (!db) throw new Error("Task database unavailable (better-sqlite3 not installed)");
...
function initDb(dbPath) {
  ...
  try {
    ...
    console.warn("[tasks] better-sqlite3 not installed — initDb unavailable");
    return null;
  }
