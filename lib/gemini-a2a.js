...
// Path: ~/.klaudii/data/chats/stream-gemini-a2a-{workspace}-{sessionNum}.jsonl
function rawLogPath(workspace, sessionNum) {
  const suffix = sessionNum != null ? `-${sessionNum}` : "";
  return path.join(CHATS_DIR, `stream-gemini-a2a-${workspace}${suffix}.jsonl`);
}
function appendRawLog(workspace, sessionNum, event) {
  try {
    fs.appendFileSync(rawLogPath(workspace, sessionNum), JSON.stringify(event) + "\n");
  } catch { /* non-fatal */ }
  dbg(`<< a2a-event workspace=${workspace} session=${sessionNum}`, JSON.stringify(event));
}

// ─── Server state per workspace × session ────────────────────────────────────
// workspace → Map<sessionNum, { proc, port, taskId, contextId, killed, reqId, agent }>
const servers = new Map();

function getServer(workspace, sessionNum) {
  const wsMap = servers.get(workspace);
  if (!wsMap) return null;
  return wsMap.get(sessionNum) || null;
}

function setServer(workspace, sessionNum, entry) {
  let wsMap = servers.get(workspace);
  if (!wsMap) { wsMap = new Map(); servers.set(workspace, wsMap); }
  wsMap.set(sessionNum, entry);
}

function deleteServer(workspace, sessionNum) {
  const wsMap = servers.get(workspace);
  if (!wsMap) return;
  wsMap.delete(sessionNum);
  if (wsMap.size === 0) servers.delete(workspace);
}

/** Find a server entry: try exact session first, fall back to any active. */
function findServer(workspace, sessionNum) {
  const exact = getServer(workspace, sessionNum);
  if (exact && !exact.killed) return exact;
  // Fallback: any active server in this workspace
  const wsMap = servers.get(workspace);
  if (!wsMap) return null;
...
/** Get info about all active servers in a workspace. */
function getActiveServerInfo(workspace) {
  const wsMap = servers.get(workspace);
  if (!wsMap) return [];
  const result = [];
  for (const [sNum, entry] of wsMap) {
    if (!entry.killed) result.push({ sessionNum: sNum, port: entry.port, pid: entry.proc.pid });
  }
  return result;
}
...
function mapA2AEvent(event, ctx = {}) {
  ...
  switch (coderKind) {
    ...
    case "tool-call-confirmation": {
      ...
      // In autoExecute (yolo) mode the A2A server auto-approves most tools,
      // but ask_user is carved out (patched in task.js) and still needs approval.
      if (!ctx.autoExecute || rawName === "ask_user") result.awaiting_approval = true;
      return result;
    }
...
async function ensureServer(workspace, sessionNum, workspacePath, config, opts) {
  const autoExecute = opts.autoExecute ?? false;
  const existing = getServer(workspace, sessionNum);
  if (existing && !existing.killed) {
    if (existing.autoExecute !== autoExecute) {
      log(`autoExecute changed (${existing.autoExecute}→${autoExecute}), restarting server workspace=${workspace} session=${sessionNum}`);
      stopProcess(workspace, sessionNum);
    } else {
      log(`reusing server workspace=${workspace} session=${sessionNum} port=${existing.port}`);
      return existing;
    }
  }
  ...
  log(`spawning a2a-server workspace=${workspace} session=${sessionNum} port=${port} script=${serverScript}`);
  ...
  proc.on("close", (code) => {
    log(`server closed workspace=${workspace} session=${sessionNum} pid=${proc.pid} code=${code}`);
    const entry = getServer(workspace, sessionNum);
    if (entry && entry.proc === proc) {
      entry.killed = true;
      deleteServer(workspace, sessionNum);
    }
    if (earlyExitReject) {
  ...
  log(`server ready workspace=${workspace} session=${sessionNum} port=${port}`);

  const { taskId, contextId } = await createTask(port, workspacePath, opts.autoExecute ?? false);
  log(`task created workspace=${workspace} session=${sessionNum} taskId=${taskId} contextId=${contextId}`);
  ...
  const entry = { proc, port, taskId, contextId, killed: false, reqId: 0, autoExecute, agent, sessionNum };
  setServer(workspace, sessionNum, entry);
  return entry;
}
...
function startChat(workspace, sessionNum, workspacePath, userMessage, config, opts = {}) {
  ...
  setImmediate(async () => {
    try {
      const entry = await ensureServer(workspace, sessionNum, workspacePath, config, opts);
      if (killed) return;
      ...
      activeReq = streamMessage(
        ...
        (a2aEvent) => {
          if (killed) return;
          appendRawLog(workspace, sessionNum, a2aEvent);
...
/**
 * Check if a workspace (or specific session) has an active A2A server.
 * Without sessionNum: returns true if ANY session is active.
 * With sessionNum: returns true only for that session.
 */
...
/**
 * Stop the A2A server for a workspace.
 * With sessionNum: stops only that session's server.
 * Without sessionNum: stops ALL servers for the workspace.
 */
function stopProcess(workspace, sessionNum) {
  if (sessionNum != null) {
    ...
    if (entry && !entry.killed) {
      entry.killed = true;
      log(`stopping server workspace=${workspace} session=${sessionNum} pid=${entry.proc.pid}`);
  ...
  for (const [sNum, entry] of [...wsMap]) {
    if (!entry.killed) {
      entry.killed = true;
      log(`stopping server workspace=${workspace} session=${sNum} pid=${entry.proc.pid}`);
      try { process.kill(-entry.proc.pid, "SIGKILL"); } catch {
        try { entry.proc.kill("SIGKILL"); } catch {}
      }
      if (entry.agent) { try { entry.agent.destroy(); } catch {} }
 ...
 */
function confirmToolCall(workspace, sessionNum, callId, outcome = "proceed_once", answer = null) {
  const entry = findServer(workspace, sessionNum);
  if (!entry) throw new Error(`No active server for workspace: ${workspace} session: ${sessionNum}`);
  ...
  const requestId = ++entry.reqId;
  log(`confirmToolCall workspace=${workspace} session=${sessionNum} callId=${callId} outcome=${outcome} answer=${JSON.stringify(answer)} reqId=${requestId}`);
  ...
  const data = { callId, outcome };
  if (answer) data.answers = answer;
  const parts = [];
  // Send user's answers as a text part so the LLM sees them even if the tool
  // already auto-executed (yolo mode completes ask_user with empty answers).
  if (answer) {
    const answerText = typeof answer === "object"
      ? Object.entries(answer).map(([, v]) => v).join("\n")
      : String(answer);
    parts.push({ kind: "text", text: answerText });
  }
  parts.push({ kind: "data", data });
  log(`confirmToolCall sending parts: ${JSON.stringify(parts)}`);
  streamParts(
...
/**
 * Execute a slash command on the running A2A server.
 * Uses the /executeCommand endpoint exposed by gemini-cli-a2a-server.
 */
async function executeCommand(workspace, sessionNum, command, args) {
  const entry = findServer(workspace, sessionNum);
  if (!entry) {
    throw new Error(`No active server for workspace: ${workspace} session: ${sessionNum}`);
  }
  return httpPost(entry.port, "/executeCommand", { command, args: args || [] }, entry.agent);
}
