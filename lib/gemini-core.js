 ...
 *
 * Imports gemini-cli-core directly and runs the GeminiClient +
 * CoreToolScheduler in-process. Exports: startChat, isActive, stopProcess,
...
const USER_SETTINGS_PATH = path.join(os.homedir(), ".gemini", "settings.json");

function resolveEnvVars(obj) {
  if (obj === null || obj === undefined || typeof obj === "boolean" || typeof obj === "number") return obj;
  if (typeof obj === "string") {
    return obj.replace(/\$(?:(\w+)|\{([^}]+)\})/g, (match, v1, v2) => {
      const name = v1 || v2;
  ...
  if (typeof obj === "object") {
    const out = { ...obj };
    for (const k of Object.keys(out)) out[k] = resolveEnvVars(out[k]);
    return out;
  }
  return obj;
}
...
function readJsonWithComments(filePath) {
  ...
  // Strip // and /* */ comments (simple inline strip — handles the common cases)
...
function loadSettings(workspaceDir) {
  let user = {}, workspace = {};
  try { user = resolveEnvVars(readJsonWithComments(USER_SETTINGS_PATH)); } catch (e) { log("settings load error (user):", e.message); }
  const wsPath = path.join(workspaceDir, ".gemini", "settings.json");
  try { workspace = resolveEnvVars(readJsonWithComments(wsPath)); } catch (e) { log("settings load error (workspace):", e.message); }
...
async function ensureSession(workspace, sessionNum, workspacePath, opts = {}) {
  ...
  try {
    ...
    log(`ensureSession autoExecute=${autoExecute} approvalMode=${JSON.stringify(approvalMode)} ApprovalMode=${JSON.stringify(core.ApprovalMode)}`);
...
/**
 * Confirm a pending tool call. This doesn't return a new event
...
async function executeCommand(workspace, sessionNum, command, args) {
  ...
  switch (command) {
    ...
    case "settings": {
      ...
      const settingsData = workspacePath ? loadSettings(workspacePath) : {};
