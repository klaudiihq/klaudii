...
function getStatus(dir) {
  try {
    ...
    try {
      ...
      // No upstream configured — count all commits as unpushed
      try {
        const total = execSync("git rev-list --count HEAD", {
          cwd: dir, encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"],
        }).trim();
        unpushed = parseInt(total) || 0;
      } catch {}
    }
...
module.exports = { isGitRepo, cloneRepo, listWorktrees, addWorktree, removeWorktree, getCurrentBranch, scanRepos, getStatus };
