...
function cleanWorktree(worktreePath, baseBranch) {
  // Safety: refuse to clean if this is the main repo root (has .git directory, not file)
  const dotGit = path.join(worktreePath, ".git");
  if (!fs.existsSync(dotGit)) {
    throw new Error(`Not a git repository: ${worktreePath}`);
  }
  if (fs.statSync(dotGit).isDirectory()) {
    throw new Error(`Refusing to clean main repo root: ${worktreePath}`);
  }

  const branch = getCurrentBranch(worktreePath);
  const base = baseBranch || "main";

  console.log(`[cleanWorktree] Cleaning worktree at ${worktreePath} (branch: ${branch}, base: ${base})`);

  execSync("git reset --hard", { cwd: worktreePath, stdio: "pipe" });
  console.log("[cleanWorktree] git reset --hard done");

  execSync("git clean -fd", { cwd: worktreePath, stdio: "pipe" });
  console.log("[cleanWorktree] git clean -fd done");

  execSync("git fetch origin", { cwd: worktreePath, stdio: "pipe", timeout: 30000 });
  console.log("[cleanWorktree] git fetch origin done");

  const ref = `origin/${base}`;
  execSync(`git checkout -B ${JSON.stringify(branch)} ${JSON.stringify(ref)}`, {
    cwd: worktreePath,
    stdio: "pipe",
  });
  console.log(`[cleanWorktree] Checked out ${branch} from ${ref}`);
}
...
module.exports = { isGitRepo, cloneRepo, initRepo, listWorktrees, addWorktree, removeWorktree, getCurrentBranch, scanRepos, getStatus, getRemoteUrl, cleanWorktree };
