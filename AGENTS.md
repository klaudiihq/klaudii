...
Workers operate in **isolated worktrees** (e.g. `klaudii--task-xyz/`). Destructive git commands in a worktree are safe because they cannot affect the main repo's working directory.
...
<!-- BEGIN TASKS INTEGRATION -->
## Issue Tracking with bd (tasks)

**IMPORTANT**: This project uses **bd (tasks)** for ALL issue tracking. Do NOT use markdown TODOs, task lists, or other tracking methods.
...
- Exports to `.tasks/issues.jsonl` after changes (5s debounce)
