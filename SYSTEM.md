...
Reads Claude CLI's own history file (`~/.claude/history.jsonl`) to provide per-workspace session history. Each entry contains a session ID, project path, timestamp, and display text. The history UI lets users resume any previous session.
