...
test.describe("Chat Overlay", () => {
  ...
  test("sending a message renders user bubble and gets response", async ({ page }) => {
    ...
    await page.waitForFunction(() => {
      return typeof chatWs !== "undefined" && chatWs && chatWs.readyState === WebSocket.OPEN;
    }, { timeout: 5000 });

    // Type and send a message
    await page.fill("#chat-input", "Test message from E2E");
  ...
  test("sending a message renders tool use indicator", async ({ page }) => {
    ...
    await page.fill("#chat-input", "Show me a file");
    await page.click("#chat-send");
  ...
  test("session dropdown button is visible", async ({ page }) => {
    await clickCard(page, "card-nova-frontend");
    await page.waitForSelector("#chat-overlay:not(.hidden)", { timeout: 5000 });

    const sessionBtn = page.locator("#chat-session-btn");
